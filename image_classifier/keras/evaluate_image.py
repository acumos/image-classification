...
def get_processed_image_cv(img_path):
...
def get_processed_image_skimage(img_path, target_size=(299,299,3)):
...
def get_processed_image_keras(img_path, target_size=(299,299,3)):

    image = image_utils.load_img(img_path, target_size=target_size)
    image = image_utils.img_to_array(image)

    # TODO: crop image, a la central_crop?

    im = inception_v4.preprocess_input(image)

    if K.image_data_format() == "channels_first":
        im = np.transpose(im, (2, 0, 1))
        im = im.reshape(-1, 3, target_size[0], target_size[1])
    else:
        im = im.reshape(-1, target_size[0], target_size[1], 3)
    return im
...
def main():
    ...
    img = get_processed_image_keras(img_path)
